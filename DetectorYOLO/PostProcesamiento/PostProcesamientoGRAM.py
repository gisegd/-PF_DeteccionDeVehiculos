...
dataSet_anotaciones='PostProcesamiento/9041.xml' 
dataSet_imagenes='image009042.jpg'
detecciones = 'PostProcesamiento/image009042.txt'
#Tamaño de la imagen (720x1200 para M-30-HD)
alto = 720 
ancho = 1200 
...
for car in root.iter('object'):
    #Las motos son ignoradas
    if(car.find('class').text == 'motorbike'): 
        continue
    box = car.find('bndbox')
    #Recorte superior e inferior
    if(int(box.find('ymin').text) < recorteYsup or int(box.find('ymax').text) > recorteYinf or int(box.find('xmax').text) < recorteX ):
        continue
